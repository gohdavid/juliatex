...
Here $Av$ is the directional derivative of the vector field along the probe direction, and the outer product with $v^T$ contracts that directional response to one scalar.
The independent Rademacher signs cancel cross terms $A_{ij}v_i v_j$ for $i\neq j$ in expectation, while each diagonal term keeps $v_i^2=1$, leaving exactly $\sum_i A_{ii}=\operatorname{Tr}(A)$.
For bead coordinates, the flattened index $i$ represents a bead-coordinate pair $(a,k)$, so this trace sums the response of $f_{\theta,a,k}$ to its matching input $x_{a,k}$.
In the SciML code path, \texttt{\_cnf\_problem} samples one $\{-1,+1\}$ probe with the same shape as the batch coordinates before constructing the \texttt{ODEProblem}.
The ODE RHS closes over that probe, so adaptive substeps, rejected steps, and adjoint replay all use the same stochastic trace direction for the solve; changing probes at every RHS call would instead add artificial time-dependent noise to the ODE.
...
\begin{equation}
    ...
    \nabla_{x_t}\log p_{0t}(x_t|x_0),
\end{equation}
and the diffusion network $s_\theta(x_t,t)$ learns to approximate this score from noisy samples.
...
\begin{equation}
    ...
    \end{aligned}
\end{equation}
Substituting $q_s(y)=p_t(y)$ and matching this equation to $\partial_s q_s=\nabla_y\cdot J_t(y)$ gives
\begin{equation}
    \tilde f(y,s)p_t(y)
    =
    -
...
If we keep the original time variable $t$ and integrate with $dt<0$, the equivalent reverse-time SDE is
\begin{equation}
    dX_t
    =
    \left[
